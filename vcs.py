...
class Repository:
    ...
    def rollback_file(self, file_name, version):
        ...
        versioned_file = FileVersion(file_name, version, self.repo_dir)
        if not versioned_file.restore_file():
...
class FileVersion:
    ...
    def restore_file(self, version_name):
        ...
        if not os.path.exists(version_file):
            print(f"Error: Version '{version_name}' does not exist for file '{self.file_path}'")
            return False
        with zipfile.ZipFile(version_file, "r") as zf:
            zf.extract(os.path.basename(self.file_path), os.path.dirname(self.file_path))
        print(f"File '{self.file_path}' restored to version '{version_name}'")
        return True
